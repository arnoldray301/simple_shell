...
ssize_t read_command(char** command, FILE* file) {
    ssize_t bytes_read;
    size_t bufsize = 0;

    bytes_read = getline(command, &bufsize, file);
    if (bytes_read == -1) {
        return -1;
...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        bytes_read = read_command(&command, file);
        if (bytes_read == -1) {
            break;
        }
        ...
        if (comment_pos != NULL) {
            *comment_pos = '\0';  /* Remove the comment portion of the command */
        }
