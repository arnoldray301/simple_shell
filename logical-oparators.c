...
int execute_commands(char *commands, char *path)
{
    ...
    char *saveptr;
    int status;
    int previous_status = 0;  /* Track the exit status of the previous command*/
    ...
    while (token != NULL)
    {
        ...
        if (strlen(command) > 0)
        {
            ...
            else
            {
                ...
                if (status == 0 && strcmp(token, "&&") == 0) {
                    /* Skip the next command if the previous one failed*/
                    break;
                } else if (status != 0 && strcmp(token, "||") == 0) {
    ...
    return previous_status;  /* Return the exit status of the last command*/
}
...

{
    ...
    int status = execute_commands(command, path);
